...
| 目標スケジューラ (`/api/ai/schedule-goal`) | 目標・本番予定を具体的な作業セッションへ分解 | 汎用的な整理・準備・最終確認へ分解 |
...
- `lib/domain/goal-planner.ts`: AIが分解した作業を空き時間へ決定論的に配置
- `app/api/calendar/write-plan/route.ts`: 衝突再検査、冪等なGoogle Calendar登録
