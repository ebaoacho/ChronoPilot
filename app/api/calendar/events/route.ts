...
export async function GET(request: Request) {
  try {
    ...
    if (user.demo) return NextResponse.json({ events: [], connected: false, lastSyncedAt: null });
    ...
    return NextResponse.json({ events: events ?? [], connected: Boolean(connection), lastSyncedAt: connection?.last_synced_at ?? null, writeMode: connection?.write_mode ?? "confirm" });
  } catch (error) {
