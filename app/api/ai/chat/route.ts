...
export async function POST(request: Request) {
  try {
    ...
    if (!user.demo) {
      ...
      const [{ data: events }, { data: planBlocks }, { data: connection }] = await Promise.all([
      ...
      if (connection) {
        try {
          ...
          const liveEvents = await listGoogleBusy({
            accessToken,
            calendarIds: connection.selected_calendar_ids?.length ? connection.selected_calendar_ids : ["primary"],
            start: horizonStart,
            end: horizonEnd
          });
      ...
      const internalBlocks = (planBlocks ?? []).map((block) => ({
        title: block.title,
        kind: block.kind as typeof serverBlocks[number]["kind"],
    ...
    raw.blocks = selectCoachBlocks([...mergedBlocks.values()], now, 100);
